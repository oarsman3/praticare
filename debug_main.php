...
<head>
  ...
  <link type="text/css" rel="stylesheet" href="praticare.php">
	...
	<form method="post" action="ricerche12.php">
		...
		<div class="form_indent">
			...
			<span class="campo">
				<select id="Contatto" name="Contatto" />
					<?php
					$result = mysqli_query($dbc,"SELECT Anagrafica_Contatto.Identificativo FROM Anagrafica_Contatto ORDER BY Anagrafica_Contatto.Identificativo ")
		...
		<div class="form_indent">
			...
			<span class="campo">
				<select id="Referente" name="Referente" />
					<?php
					$result = mysqli_query($dbc,"SELECT Anagrafica_Referente.Cognome_Ref FROM Anagrafica_Referente ORDER BY Anagrafica_Referente.Cognome_Ref ")
					...
					while($row_list=mysqli_fetch_array($result,MYSQLI_BOTH)) {
					?>
						<option " <?php echo $row_list['Cognome_Ref']; ?> " ><?php echo $row_list['Cognome_Ref']; ?></option>
					<?php
					}
					?>
				</select>
		...
		<div class="form_indent">
			...
			<span class="campo">
				<select id="Bambino" name="Bambino" />
					<?php
					$result = mysqli_query($dbc,"SELECT Bambini.Cognome FROM Bambini ORDER BY Bambini.Cognome ")
					...
					while($row_list=mysqli_fetch_array($result,MYSQLI_BOTH)) {
					?>
						<option " <?php echo $row_list['Cognome']; ?> " ><?php echo $row_list['Cognome']; ?></option>
					<?php
					}
					?>
				</select>
		...
		<br />
		PIVOT        <input name="Pivot" type="checkbox"/>
		<?php
		mysqli_close($dbc);
		?>
